...
p <- ggplot(data=df.sample, aes(x=effect_size, y=sample_size)) + geom_point() + scale_y_continuous(breaks = scales::pretty_breaks(n = 20)) + scale_x_continuous(breaks = scales::pretty_breaks(n = 20)) + 
  ...
  geom_vline(xintercept=0.7, color='red') + annotate("text", x=0.8, y=150, label="TEPR", color='red')
